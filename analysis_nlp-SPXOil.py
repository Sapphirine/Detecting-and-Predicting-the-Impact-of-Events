...
sql_stmt = 'select newsdate, title, fulltext from import_news_articles where newsdate between \'2012-11-1\' and \'2012-11-30\' '
...
for index, row in outputDF.iterrows():
  if 'BP' in row['title'] or 'Exxon' in row['title']:
    countDF['tcount'][row['newsdate'].date()] = countDF['tcount'][row['newsdate'].date()] + 1
  if row['fulltext'] is not None and ('Exxon' in row['fulltext'] or 'BP' in row['fulltext']):
...
sql_stmt = 'select tickdate, value from import_raw_ticks where tickdate between \'2012-11-1\' and \'2012-11-30\' and security = \'S5OILG Index\' and field = \'PX_LAST\' order by tickdate'
if True:
  conn = psycopg2.connect(database="postgres", user = "postgres", password = "swapnil", host = "35.190.146.57", port = "5432")
  cur = conn.cursor()
...
ax1.set_xlabel('S5OILG Index (Industry) Oil Gas and Consumable Industry Vs Close Price')
ax1.set_ylabel('Titles + Full', color='b')
ax1.tick_params('y', colors='b')

ax2 = ax1.twinx()
ax2.plot(countDF.index, countDF['tickvalue'],'g-')
ax2.set_ylabel('Close Price', color='g')
